...
class UI:
    ...
    def draw_sidebar(self, game):
        ...
        self.restart_button_rect = pygame.Rect(sidebar_x + BUTTON_PADDING, y_offset,
                                               SIDEBAR_WIDTH - 2 * BUTTON_PADDING, BUTTON_HEIGHT)
        pygame.draw.rect(self.screen, ORANGE, self.restart_button_rect)
        pygame.draw.rect(self.screen, BLACK, self.restart_button_rect, 2)
        
        restart_text = self.font.render("Restart Level", True, BLACK)
        ...
        button_size = 35
        button_spacing = 5
        types = ['basic', 'sniper', 'rapid', 'cannon', 'dog']
        type_labels = ['B', 'S', 'R', 'C', 'D']
                ...
                if event.button == 1:  # Left click
                    ...
                    if mouse_x < SCREEN_WIDTH - SIDEBAR_WIDTH:
                        if not game.game_over:
                            ...
                            if clicked_tower:
                                # If clicking on dog tower, pet it for money
                                if clicked_tower.tower_type == 'dog' and game.selected_tower == clicked_tower:
                                    game.pet_money_tower(clicked_tower)
                                    logger.info("Dog tower petted!")
                                else:
                        ...
                        # Check if clicking restart level button
                        if ui.restart_button_rect and ui.restart_button_rect.collidepoint(mouse_x, mouse_y):
                            game.restart_level()
                            logger.info("Level restarted by player")
